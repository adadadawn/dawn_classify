...
train_data= torchvision.datasets.CIFAR10('./data',train= True,transform= torchvision.transforms.ToTensor(),download=True)   #将训练集进行初始化
test_data= torchvision.datasets.CIFAR10('. /data',train= False, transform= torchvision.transforms.ToTensor(),download=True) #将测试集进行初始化
...
for d in test_dataloader:   #必须要遍历才能看数据的shape
    img, target= d
...
for times in range(10):
    ...
    with torch.no_grad():   #以下参数不会自动求导
        for data in test_dataloader:
            ...
            step+=1
        writer.add_scalar('accurancy', accurancy,step)
        writer.add_scalar('accuracy_add', accurancy_add,step)
